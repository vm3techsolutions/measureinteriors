...
// Define categories here (you can adjust this as per your API response if required)
const categories = [
  "All Products",
  "Accessories",
  "Adjustable Desk",
...
const ProductSection = () => {
  ...
  // Fetch products data from API using environment variable
  useEffect(() => {
    const fetchProducts = async () => {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL; // Fetch from environment variable
    ...
    <div className="p-6 bg-[#EDEDE9] min-h-screen pb-14">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-6 px-5">
        {filteredProducts.map(product => (
          ...
          >
            <img
              src={product.acf.image} // Assuming `acf.image` contains the image URL in ACF
              alt={product.title.rendered} // Assuming title is under 'rendered' key in API
              className="rounded-md h-40 object-cover w-full mb-4"
            />
            <h2 className="text-xl font-bold text-black">
              {product.title.rendered}
            </h2>
            <p className="text-md text-black mt-2">{product.acf.description}</p>
            <p className="text-lg mt-2 font-bold text-black">
              Category: <span className="text-black text-md font-normal">{product.acf.category}</span>
            </p>
            <a
              href={product.link}
